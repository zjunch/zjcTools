...
public class VMRecordView extends View {
    ...
    private void checkPermission() {
        // 检查录音权限
        if (ZPermission.getInstance(getContext()).checkRecord()) {
            isUsable = true;
        } else {
            isUsable = false;
            ZPermission.getInstance(getContext()).requestRecord(new ZPermission.PCallback() {
                @Override
    ...
    protected void drawUnusable(Canvas canvas) {
        ...
        float tWidth = ZDimen.getTextWidth(mPaint, mUnusableDesc);
        float tHeight = ZDimen.getTextHeight(mPaint);
    ...
    protected void drawTouch(Canvas canvas) {
        ...
        mPaint.setTextSize(mDescSize);
        float tWidth = ZDimen.getTextWidth(mPaint, desc);
        float tHeight = ZDimen.getTextHeight(mPaint);
...
//        String time = ZjcStr.byArgs("%02d'%02d''%d'''", minute, seconds, millisecond);
        ...
        float tWidth = ZDimen.getTextWidth(mPaint, time);
        float tHeight = ZDimen.getTextHeight(mPaint);
