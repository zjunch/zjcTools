...
import com.android.zjctools.utils.ZDimen.getTextHeight
import com.android.zjctools.utils.ZDimen.getTextWidth
import com.android.zjctools.utils.ZStr.byArgs
import com.android.zjctools.utils.ZStr.isEmpty
import com.android.zjctools.utils.ZSystem.runInUIThread
 ...
 * 自定义录音控件
 */
class VMRecordView @JvmOverloads constructor(
    context: Context?,
    ...
    // 控件背景颜色
    private var mCancelColor = byRes(R.color.z_red_87)

    // 外圈的颜色、大小
    private var mOuterColor = byRes(R.color.z_green_38)
    private var mOuterSize = dp2px(128)

    // 内圈录音按钮的颜色、大小
    private var mInnerColor = byRes(R.color.zGreen)
    private var mInnerSize = dp2px(96)

    // 触摸区域提示文本
    private var mDescNormal: String? = "触摸录音"
    private var mDescCancel: String? = "松开取消"
    private var mDescColor = byRes(R.color.zWhite)
    private var mDescSize = dp2px(16)

    // 时间字体的大小、颜色
    private var mTimeColor = byRes(R.color.zGreen)
    private var mTimeSize = dp2px(14)
    ...
    private fun handleAttrs(attrs: AttributeSet?) {
        ...
        val array = context.obtainStyledAttributes(attrs, R.styleable.VMRecordView)
        ...
        mInnerSize =
            array.getDimensionPixelOffset(R.styleable.VMRecordView_zv_inner_size, mInnerSize)
        ...
        if (isEmpty(mDescNormal)) {
            mDescNormal = "触摸录音"
        }
        if (isEmpty(mDescCancel)) {
            mDescCancel = "松开取消"
        }
    ...
    private fun drawBackground(canvas: Canvas) {
        if (isCancelRecord) {
            ...
            mPaint!!.color = byRes(R.color.zTransparent)
        }
        // 绘制背景
    ...
    protected fun drawUnusable(canvas: Canvas) {
        ...
        val tWidth = getTextWidth(mPaint!!, mUnusableDesc)
        val tHeight = getTextHeight(mPaint!!)
        canvas.drawText(
            mUnusableDesc, mWidth / 2 - tWidth / 2, mHeight / 2 + tHeight / 3,
            mPaint!!
        )
    }
    ...
    protected fun drawTouch(canvas: Canvas) {
        if (isStartRecord && !isCancelRecord) {
            ...
            canvas.drawCircle(
                (mWidth / 2).toFloat(), (mHeight / 2).toFloat(), (mOuterSize / 2).toFloat(),
                mPaint!!
            )
        }
        val innerColor: Int
        ...
        val desc: String?
        if (isCancelRecord) {
            innerColor = byRes(R.color.zWhite)
        ...
        canvas.drawCircle(
            (mWidth / 2).toFloat(), (mHeight / 2).toFloat(), (mInnerSize / 2).toFloat(),
            mPaint!!
        )
        ...
        mPaint!!.textSize = mDescSize.toFloat()
        val tWidth = getTextWidth(mPaint!!, desc)
        val tHeight = getTextHeight(mPaint!!)
...
//        String time = ZjcStr.byArgs("%02d'%02d''%d'''", minute, seconds, millisecond);
    ...
    private fun startOuterAnim() {
        runInUIThread {
            val mAnimator = ValueAnimator.ofInt(
                mInnerSize,
                mInnerSize + mDecibel * mHeight / 10,
                mInnerSize
            )
