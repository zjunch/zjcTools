 ...
 * 扫描图片工具类
 */
public class ZPickScanPicture implements LoaderManager.LoaderCallbacks<Cursor> {
    // 加载所有图片
    ...
    // 所有的图片文件夹
    private ArrayList<ZFolderBean> mFolderBeans = new ArrayList<>();
     ...
     */
    public ZPickScanPicture(FragmentActivity activity, String path, OnScanPictureListener listener) {
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        if (data != null) {
            ...
            for (data.moveToFirst(); !data.isAfterLast(); data.moveToNext()) {
                ...
                if (ZBitmap.getBitmapDegree(imagePath) == 90 || ZBitmap.getBitmapDegree(imagePath) == 270) {
        ...
        //回调接口，通知图片数据准备完成
        ZPicker.getInstance().setFolderBeans(mFolderBeans);
    ...
    public interface OnScanPictureListener {
        void onLoadComplete(List<ZFolderBean> VMFolderBeans);
    }
}
