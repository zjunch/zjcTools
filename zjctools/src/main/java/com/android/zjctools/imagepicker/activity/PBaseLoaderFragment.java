package com.android.zjctools.imagepicker.activity;
...
import static com.android.zjctools.imagepicker.ImagePicker.REQ_CAMERA;
import static com.android.zjctools.imagepicker.ImagePicker.REQ_STORAGE;
    ...
    public void takeVideo() {
        ...
        if (ContextCompat.checkSelfPermission(getActivity(), Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
            ...
            ImagePicker.takeVideo(getActivity(), null, getSelectConfig().getMaxVideoDuration(),
                    false, new OnImagePickCompleteListener() {
                        @Override
                        public void onImagePickComplete(ArrayList<ImageItem> items) {
                            if (items != null && items.size() > 0 && items.get(0) != null) {
                                onTakePhotoResult(items.get(0));
