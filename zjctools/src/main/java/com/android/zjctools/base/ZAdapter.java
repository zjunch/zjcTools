...
public abstract class ZAdapter<T, VH extends ZHolder> extends RecyclerView.Adapter<ZHolder> {
    ...
    @NonNull
    @Override
    public ZHolder onCreateViewHolder(@NonNull ViewGroup root, int viewType) {
        if (mHeaderViews.get(viewType) != null) {
            return new ZHolder(mHeaderViews.get(viewType));
        }
        if (isEmpty() && viewType == ITEM_TYPE_EMPTY) {
            ...
            return new ZHolder(mEmptyView);
        }
        if (mFooterViews.get(viewType) != null) {
            return new ZHolder(mFooterViews.get(viewType));
        }
        if (viewType == ITEM_TYPE_MORE) {
            return new ZHolder(mMoreView);
        }
        return createHolder(root, viewType);
    }

    @Override
    public void onBindViewHolder(@NonNull ZHolder holder, final int position) {
