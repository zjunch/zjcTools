...
object ZDate {
     ...
     * @param dateStr 原来的时间
     */
    fun converDateTime(srcFormat: String?, desFormat: String?, dateStr: String?): String? {
        return try {
    ...
    fun fromTimeString(time: String): Long {
        ...
        val split = time.split(":".toRegex()).toTypedArray()
    ...
    fun covertToDate(dateTime: String?): String {
        ...
        try {
            ...
            df2 = SimpleDateFormat("yyyy-MM-dd")
        } catch (e: ParseException) {
            e.printStackTrace()
        }
